...
func TestBollingerBands(t *testing.T) {
  ...
  if gotUBand != wantUBand {
    t.Errorf("bollinger.BollingerBandSeries(%v).Upper = %v, want %v", testPrices, gotUBand, wantUBand)
  }

  gotLBand := bands.Lower[gotLen-1]
  wantLBand := 1167.9746157650914
  if gotLBand != wantLBand {
    t.Errorf("bollinger.BollingerBandSeries(%v).Upper = %v, want %v", testPrices, gotLBand, wantLBand)
  }

  gotMA := bands.MA[gotLen-1]
  wantMA := 1214.1639999999998
  if gotMA != wantMA {
    t.Errorf("bollinger.BollingerBandSeries(%v).Upper = %v, want %v", testPrices, gotMA, wantMA)
  }

  wantLen := 7
  if wantLen != gotLen {
    t.Errorf("len(bollinger.BollingerBandSeries(%v)) = %v, want %v", testPrices, gotLen, wantLen)
  }
}
